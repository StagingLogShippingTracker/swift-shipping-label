...
def run_loop(skip_render: bool = False, top_n: int = 10) -> dict:
    ...
    summary = {
        ...
        "next": (
            "Read this summary; fix top_failures that are real layout bugs "
            "(overflow, logo encroachment, margins, columns, bars) without "
            "changing locked SO/Contact constants; re-run this script. "
            "If only gate_fails on approved bands, adjust shipping_label_score.py."
        ),
    }
    SUMMARY.write_text(json.dumps(summary, indent=2), encoding="utf-8")
