...
void main() {
  ...
  test('generate Shipping + Receiving samples with Helvetica entry fields',
      () async {
    final shipping = await ShippingLabelPdf.load();
    final root = Directory.current.parent.path;
    final sep = Platform.pathSeparator;
    const options = PdfRenderOptions(bodyFont: PdfBodyFont.helvetica);
    ...
    final shipOut = File(
      '$root${sep}Swift Supply Shipping Label - Sample (Helvetica).pdf',
    );
    ...
    final recvOut = File(
      '$root${sep}Swift Supply Receiving Label - Sample (Helvetica).pdf',
    );
