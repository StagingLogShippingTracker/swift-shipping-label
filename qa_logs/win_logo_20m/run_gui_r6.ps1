# Real Windows-app Find-logo QA (r6): always Tools+Down*6+Enter; click field; strict picker.
param(
...
$roundDir = Join-Path $outDir "gui_r6"
...
foreach ($company in $companies) {
  ...
  @{ started=$start.ToString('o'); ended=(Get-Date).ToString('o'); companies=$results } |
    ConvertTo-Json -Depth 6 | Set-Content (Join-Path $outDir "gui_r6.json")
